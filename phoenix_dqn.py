...
from phoenix_core import Agent
...
class AbstractDQNAgent(Agent):
    """Write me
    """
    def __init__(self, nb_actions, memories, gamma=.99, batch_size=32, nb_steps_warmup=1000,
...
class DQNAgent(AbstractDQNAgent):
    ...
    def __init__(self, models, policy=None, test_policy=None, enable_double_dqn=True, enable_dueling_network=False,
        ...
        for i in range(len(models)):
            if hasattr(models[i].output, '__len__') and len(models[i].output) > 1:
                raise ValueError('Model "{}" has more than one output. DQN expects a model that has a single output.'.format(models[i]))
            if models[i].output._keras_shape != (None, self.nb_actions):
                raise ValueError('Model output "{}" has invalid shape. DQN expects a model that has one dimension for each action, in this case {}.'.format(models[i].output, self.nb_actions))
        ...
        if self.enable_dueling_network:
            for i in range(len(models)):
                ...
                layer = models[i].layers[-2]
                nb_action = models[i].output._keras_shape[-1]
                ...
                models[i] = Model(inputs=models[i].input, outputs=outputlayer)
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        for i in range(len(self.models)):
            ...
            self.target_models.append(clone_model(self.models[i], self.custom_model_objects))
            ...
            if self.target_model_update < 1.:
                ...
                updates = get_soft_target_model_updates(self.target_models[i], self.models[i], self.target_model_update)
            ...
            y_pred = self.models[i].output
            y_true = Input(name='y_true', shape=(self.nb_actions,))
            mask = Input(name='mask', shape=(self.nb_actions,))
            loss_out = Lambda(clipped_masked_error, output_shape=(1,), name='loss')([y_true, y_pred, mask])
            ins = [self.models[i].input] if type(self.models[i].input) is not list else self.models[i].input
        ...
        self.trainable_model = self.trainable_models[0]
        self.compiled = True

    def load_weights(self, filepath):
        for i in range(len(self.models)):
            self.models[i].load_weights(filepath+'{}'.format(i))
        self.update_target_models_hard()

    def preload_weights(self, filepath):
        for i in range(len(filepath)):
            self.models[i].load_weights(filepath[i])
        for i in range(len(filepath)):
            self.target_models[i].set_weights(self.models[i].get_weights())

    def save_weights(self, filepath, overwrite=False):
        for i in range(len(self.models)):
            self.models[i].save_weights(filepath+'{}'.format(i), overwrite=overwrite)
    ...
    def update_invalid_actions(self, invalid_actions, actions):
        for i in range(len(actions)):
            if actions[i] not in invalid_actions:
                invalid_actions.append(actions[i])
            
    def forward(self, observation):
        self.memory = self.memories[int(self.processor.level/2)]
        self.model = self.models[int(self.processor.level/2)]
        self.target_model = self.target_models[int(self.processor.level/2)]
        self.trainable_model = self.trainable_models[int(self.processor.level/2)]
        
        # Select an action.
        state = self.memory.get_recent_state(observation)
        q_values = self.compute_q_values(state)
        valid_actions = np.arange(self.nb_actions)
        
        # Remove invalid actions
        invalid_actions = []
        
        if observation[5]==127:
            self.update_invalid_actions(invalid_actions, [2, 3, 4, 5, 6, 7])
        elif observation[6]!=-128:
            self.update_invalid_actions(invalid_actions, [4, 7])

        if observation[2]==127:
            self.update_invalid_actions(invalid_actions, [1, 5, 6, 7])

        valid_actions = np.delete(valid_actions, invalid_actions)
        q_values = np.delete(q_values, invalid_actions)
        
        action_index = 0
            
        if self.training and self.processor.level in self.training_levels:
            action_index = self.policy.select_action(q_values=q_values)
        else:
            action_index = self.test_policy.select_action(q_values=q_values)
    ...
    def backward(self, reward, terminal):
        ...
        if not self.processor.alive or self.processor.time_limit>100:
            if self.training_levels == []:
                self.processor.record = max(self.processor.record, self.processor.level)
            
            temp = int(self.processor.level/2)
            if self.training_levels != [2*temp, 2*temp+1]:
                self.training_levels = [2*temp, 2*temp+1]

            if self.processor.level_limit != 2*temp+1:
                self.processor.level_limit = 2*temp+1
        ...
        #metrics = [np.nan for _ in self.metrics_names]
        if not self.training or self.processor.level not in self.training_levels:
            # We're done here. No need to update the experience memory since we only use the working
            # memory to obtain the state over the most recent observations.
            return metrics

        # Train the network on a single stochastic batch.
        if self.memory.nb_entries > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            if self.enable_double_dqn:
                ...
                for i in range(self.batch_size):
                    ...
                    # Remove invalid actions
                    invalid_actions = []
                    if state0_batch[i][0][5]==127:
                        self.update_invalid_actions(invalid_actions, [2, 3, 4, 5, 6, 7])
                    elif state0_batch[i][0][6]!=-128:
                        self.update_invalid_actions(invalid_actions, [4, 7])

                    if state0_batch[i][0][2]==127:
                        self.update_invalid_actions(invalid_actions, [1, 5, 6, 7])

                    valid_actions = np.delete(valid_actions, invalid_actions)
                    tempq = q_values[i]
                    tempq = np.delete(tempq, invalid_actions)
        
                    action_index = np.argmax(tempq)
                    actions.append(valid_actions[action_index])
